...
PROCEDURE PROC_QSTN_PRE_UPD(
    ...
    OC_CTLG_CUR       OUT     PCKG_QAHUB_CTLG.CTLG_CUR,
    OC_SUBCTLG_CUR    OUT     PCKG_QAHUB_CTLG.SUBCLTG_CUR
);
        ...
        FROM(
            ...
            FROM(
              ...
              WHERE (Q.TITLE LIKE '%'||IS_KEYWORD||'%' OR
                    ...
                    Q.SUBCTLG_ID = SC.SUBCTLG_ID AND
                    Q.DEL_FLAG = '0'
                    ORDER BY Q.ENT_TMS
              ) A
...
PROCEDURE PROC_QSTN_PRE_UPD(
    ...
    V_CTL_COUNTER     INTEGER;
    V_CTLG_ID         VARCHAR2(32);

  BEGIN

    SELECT COUNT(*) INTO V_CTL_COUNTER
      FROM QAHUB_QSTN_DTL WHERE QSTN_ID = IS_QSTN_ID;
    
    IF V_CTL_COUNTER >0 THEN
      OI_FLAG     :=-1;
    ELSE

      INSERT INTO QAHUB_QSTN_DTL (QSTN_SEQ,QSTN_ID,CTLG_ID,SUBCTLG_ID,USER_ID,TITLE,CONTENT,ATTMNT,STATUS,REC_TMS)
        SELECT SYS_GUID,QSTN_ID,CTLG_ID,SUBCTLG_ID,USER_ID,TITLE,CONTENT,STATUS,REPLY_NO,ENT_TMS 
          FROM QAHUB_QSTN WHERE QSTN_ID=IS_QSTN_ID;

      COMMIT;

      OPEN  OC_QSTN_UPD_CUR FOR
        SELECT  QSTN_ID,CTLG_ID,SUBCTLG_ID,USER_ID,TITLE,CONTENT,STATUS,REPLY_NO,ENT_DT,UPD_DT,ENT_TMS,UPD_TMS
          FROM  QAHUB_QSTN WHERE QSTN_ID=IS_QSTN_ID FOR UPDATE;

      SELECT CTLG_ID INTO V_CTLG_ID 
        FROM QAHUB_QSTN WHERE QSTN_ID=IS_QSTN_ID;

      PCKG_QAHUB_CTLG.PROC_CTLG_SUBCTLG_QRY(
        IS_CTLG_ID => V_CTLG_ID,
        OI_FLAG => OI_FLAG,
        OS_ERRCODE => OS_ERRCODE,
        OS_MSG => OS_MSG,
        OC_CTLG_CUR => OC_CTLG_CUR,
        OC_SUBCTLG_CUR => OC_SUBCTLG_CUR
      );
...
PROCEDURE PROC_QSTN_UPD(
  ...
  BEGIN
    ...
    UPDATE QAHUB_QSTN_DTL SET CTRL_FLAG='1'    
    WHERE QSTN_ID = IS_QSTN_ID AND CTRL_FLAG='0';
...
PROCEDURE PROC_ANS_UPD(
  ...
  BEGIN
    ...
    UPDATE QAHUB_ANS_DTL SET CTRL_FLAG='1'
    WHERE ANS_ID = IS_ANS_ID AND CTRL_FLAG='0';
